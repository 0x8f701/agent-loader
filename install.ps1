...
# and installs the binary as %USERPROFILE%\.agent-loader\bin\al.exe.
#
# Usage:
...
if (-not [System.Environment]::Is64BitOperatingSystem) {
    Fail "al requires 64-bit Windows (x86_64)"
}
...
$Headers = @{ "User-Agent" = "al-install"; "Accept" = "application/vnd.github+json" }
...
$Asset = "al-$ResolvedVersion-$Triple.zip"
...
# ── Download + verify ────────────────────────────────────────────────────────
$TmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("al-install-" + [System.IO.Path]::GetRandomFileName())
...
try {
    ...
    Write-Host "Downloading al v$ResolvedVersion ($Triple)..."
    ...
    $BinaryPath = Join-Path $TmpDir "al.exe"
    ...
    Ensure-SafeDirectory $AlHome "al install root"
    $BinDir = Join-Path $AlHome "bin"
    Ensure-SafeDirectory $BinDir "al bin directory"
    $Dest = Join-Path $BinDir "al.exe"
    $StatePath = Join-Path $AlHome "update-state.json"
    if (Test-Path -LiteralPath $Dest) {
        $DestItem = Get-Item -LiteralPath $Dest -Force
        if (($DestItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            Fail "refusing to replace reparse-point al executable: $Dest"
        }
        if ($DestItem.PSIsContainer) {
            Fail "refusing to replace directory at al executable path: $Dest"
        }
    }
    ...
    # A running al.exe blocks writes but may allow renames. Use a unique aside
    # path so a locked backup from an older process cannot block updates.
    ...
    if ($HadPrior) {
        try {
            ...
            Fail "cannot replace $Dest (close all running al sessions and retry): $($_.Exception.Message)"
        }
    }
    try {
    ...
    Write-Host ""
    Write-Host "al v$ResolvedVersion installed to $Dest"
    ...
    if (-not $OnPath) {
        ...
        try {
            ...
            Write-Host "Open a new terminal, then run 'al' to get started."
        } catch {
        ...
        Write-Host "Run 'al' to get started."
    }
} finally {
