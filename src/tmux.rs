...
mod platform {
    ...
    /// One named window in a multi-window tmux session created by `al new`.
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub struct NamedWindow {
        pub name: String,
        pub spec: crate::launcher::CommandSpec,
    }

    /// Create a session with one window per spec, then optionally attach.
    /// An existing session of the same name is replaced.
    pub fn run_named_windows(session: &str, windows: &[NamedWindow], attach: bool) -> Result<i32> {
        if windows.is_empty() {
            bail!("tmux-run: at least one window is required");
        }
        let mut request = exact_request(session, &windows[0].spec, None)?;
        request.no_attach = !attach;
        ...
        run_named_windows_with(&request, windows, &environment, &mut SystemExecutor)
    }

    /// Attach or switch to an existing session.
    pub fn attach_session(session: &str) -> Result<i32> {
        let request = Request {
            no_attach: false,
            fresh: false,
            session: os(session),
            window: None,
            cwd: env::current_dir().context("reading current directory for tmux attach")?,
            socket_name: None,
            socket_path: None,
            command: Vec::new(),
        ...
        let target = exact_session_target(OsStr::new(session));
        if inside_requested_tmux(&request) {
            return tmux_status(
                &request,
                &mut SystemExecutor,
                [os("switch-client"), os("-t"), target],
            );
        }
        tmux_status(
            &request,
            &mut SystemExecutor,
            [os("attach-session"), os("-t"), target],
        )
    }
    ...
    fn run_named_windows_with<E: Executor>(
        request: &Request,
        windows: &[NamedWindow],
        ...
        if has_session(request, executor, &request.session)? {
            let status = tmux_status(
                ...
                [
                    os("kill-session"),
                    os("-t"),
                    exact_session_target(&request.session),
                ],
            )?;
            if status != 0 {
                return Ok(status);
        ...
        for (index, window) in windows.iter().enumerate() {
            let argv = ExactCommand::from_spec(&window.spec).into_argv();
            let launch = ChildLaunch::create(&environment.shell, CommandMode::Argv, &argv, None)?;
            ...
            let status = tmux_status(request, executor, command)?;
            if status != 0 {
                if index > 0 {
                    kill_session(request, executor, &request.session);
                }
                return Ok(status);
            }
            let mut target = exact_session_target(&request.session);
            target.push(":");
            target.push(&window.name);
            target.push(".0");
    ...
    fn run_with<E: Executor>(
        ...
        if current_session.as_ref() == Some(&request.session) && request.fresh && request.no_attach
        {
        ...
        if environment.inside_tmux {
            let status = tmux_status(
                request,
                executor,
                [os("switch-client"), os("-t"), request.session.clone()],
            )?;
            if status != 0 {
                if swap_current {
                    let old_session = old_session.as_ref().expect("swap has old session");
            ...
            tmux_status(
                request,
                executor,
                [os("attach-session"), os("-t"), request.session.clone()],
            )
        }
    }
    ...
    fn exact_session_target(session: &OsStr) -> OsString {
        let text = session.to_string_lossy();
        if text.starts_with('=') {
            session.to_owned()
        } else {
    ...
    fn has_session<E: Executor>(
        ...
        let output = tmux_capture(
            request,
            executor,
            [os("has-session"), os("-t"), exact_session_target(session)],
        )?;
        Ok(output.status == 0)
    }
    ...
    fn kill_session<E: Executor>(request: &Request, executor: &mut E, session: &OsStr) {
        let _ = tmux_status(
            request,
            executor,
            [os("kill-session"), os("-t"), exact_session_target(session)],
        );
    }

    fn tmux_capture<E, I>(request: &Request, executor: &mut E, command: I) -> Result<ExecuteOutput>
    where
        E: Executor,
    ...
    fn read_payload(path: &Path) -> Result<ChildPayload> {
        ...
        let file =
            File::open(path).with_context(|| format!("opening tmux argv payload {path:?}"))?;
        let metadata = file
            .metadata()
        ...
        if reader
            .read(&mut trailing)
            .context("checking tmux payload end")?
            != 0
        {
    ...
    trait ChildRuntime {
        fn spawn(&mut self, program: &OsStr, args: &[OsString])
    ...
    fn run_child_with<R: ChildRuntime>(
        payload: &Path,
        ready: &Path,
        runtime: &mut R,
    ) -> Result<i32> {
    ...
    fn report_early_exit<R: ChildRuntime>(
        ready: &Path,
        status: i32,
        runtime: &mut R,
    ) -> Result<i32> {
    ...
    mod tests {
        ...
        fn parser_derives_shell_and_program_sessions() {
            ...
            assert_eq!(
                request(&["/usr/bin/python3", "-q"]).session,
                OsStr::new("python3")
            );
        ...
        impl Executor for Recorder {
            fn execute(
                ...
                match name {
                    ...
                    "has-session" => {
                        let target =
                            strip_exact_session(option_after(&command, "-t").unwrap_or_default());
                        Ok(ExecuteOutput {
                            status: if self.sessions.contains(&target) {
                                0
                            } else {
                                1
                            },
                    ...
                    "kill-session" => {
                        if let Some(target) = option_after(&command, "-t") {
                            self.sessions.remove(&strip_exact_session(target));
                        }
                        Ok(ExecuteOutput {
                            status: 0,
                            stdout: Vec::new(),
        ...
        fn single_string_command_uses_login_shell_parsing() {
            ...
            assert_eq!(direct.args, strings(&["-lc", "echo one | sed s/one/two/"]));
        }
        ...
        fn fresh_current_session_renames_only_after_readiness_and_never_kills_current() {
            ...
            let new_index = names
                .iter()
                .position(|name| *name == "new-session")
                .unwrap();
            let first_rename = names
                .iter()
                .position(|name| *name == "rename-session")
                .unwrap();
            let switch = names
                .iter()
                .position(|name| *name == "switch-client")
                .unwrap();
            let final_kill = names
                .iter()
                .rposition(|name| *name == "kill-session")
                .unwrap();
            ...
            assert!(!commands[..switch].iter().any(|command| {
                command[0] == OsStr::new("kill-session")
                    && option_after(command, "-t")
                        .map(strip_exact_session)
                        .as_deref()
        ...
        fn early_failure_returns_status_kills_session_and_cleans_private_files() {
            let request = request(&["--no-attach", "-s", "bad", "--", "false", "spaced arg"]);
            let mut recorder = Recorder {
        ...
        fn raw_payload_round_trips_spaces_metacharacters_and_non_utf8_privately() {
            ...
            let argv = vec![os("tool name"), os("; $(touch /nope) ' \""), raw.clone()];
            write_payload(
                &path,
                OsStr::new("/bin/fish"),
                CommandMode::Argv,
        ...
        fn assert_spawn_failure(
            ...
            assert_eq!(
                runtime.marker_during_grace.as_deref(),
        ...
        fn absent_primary_executable_reports_127_through_readiness_marker() {
            let directory = tempdir().unwrap();
            assert_spawn_failure(&directory.path().join("absent"), false, 127, b"exit 127\n");
        }
        ...
        fn absent_fallback_executable_reports_127_through_readiness_marker() {
            let directory = tempdir().unwrap();
            assert_spawn_failure(&directory.path().join("absent"), true, 127, b"exit 127\n");
        }
        ...
        fn normal_child_exit_removes_marker_and_enters_selected_login_shell() {
            ...
            assert_eq!(run_child_with(&payload, &ready, &mut runtime).unwrap(), 42);
            assert_eq!(
                runtime.spawned,
                Some((os("agent"), strings(&["literal ; value", "two words"])))
            );
            assert_eq!(runtime.login, Some((os("/custom/fish"), strings(&["-li"]))));
            assert!(!payload.exists());
            assert!(!ready.exists());
        ...
        fn primary_failure_runs_fallback_as_exact_argv_before_readiness() {
            ...
            assert_eq!(run_child_with(&payload, &ready, &mut runtime).unwrap(), 0);
            assert_eq!(
                runtime.spawned,
                vec![
        ...
        fn strip_exact_session(target: OsString) -> OsString {
            match target.to_str() {
                Some(text) if text.starts_with('=') => OsString::from(&text[1..]),
                _ => target,
            }
        }
        ...
        fn read_payload_rejects_malformed_inputs() {
            ...
            *invalid_fallback_flag.last_mut().unwrap() = 2;
            ...
            for (label, bytes, expected) in cases {
                ...
                assert!(
                    result.is_err(),
                    "{label}: malformed payload must be rejected"
                );
        ...
        fn malformed_payload_never_spawns_child_runtime() {
            ...
            *invalid_fallback_flag.last_mut().unwrap() = 2;
            ...
            for (index, (label, bytes)) in cases.iter().enumerate() {
                ...
                assert!(
                    result.is_err(),
                    "{label}: malformed payload must fail closed"
                );
                assert!(
...
mod platform {
    ...
    pub fn run_named_windows(
        _session: &str,
        _windows: &[NamedWindow],
        _attach: bool,
    ) -> Result<i32> {
        unsupported()
    }

    pub fn attach_session(_session: &str) -> Result<i32> {
        unsupported()
    }
...
pub use platform::{
    CommandMode, NamedWindow, Request, attach_session, run, run_argv, run_child, run_exact,
    run_exact_fallback, run_named_windows,
};
